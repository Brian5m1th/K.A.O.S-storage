...
class ObsidianService:
    ...
    def list_notes(self, folder: str = "") -> list[str]:
        ...
        return [str(p.relative_to(self._vault_path)) for p in search_path.rglob("*.md") if not p.name.startswith(".")]
...
Cada tool é registrada via `@tool` do LangChain.
...
def search_notes(query: str, max_results: int = 5) -> dict:
    ...
    return {"query": query, "total": len(results), "documents": [r.model_dump() for r in results]}
```
...
Estratégia: usar `tmp_path` do pytest para criar um Vault temporário.
